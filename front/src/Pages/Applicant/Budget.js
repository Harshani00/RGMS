...
function Budget() {
  const [appIds, setAppIds] = useState([]);
  const [selectedAppId, setSelectedAppId] = useState(""); // Ensure it's empty initially to show the placeholder
  const [acceptedFiles1, setAcceptedFiles1] = useState([]);
  const [acceptedFiles2, setAcceptedFiles2] = useState([]);
  const [errors, setErrors] = useState({});
  const [submitted, setSubmitted] = useState(false);
    ...
    <div>
      ...
      <Form.Group controlId="appIdSelect">
        ...
        <Form.Control
          ...
          onChange={(e) => setSelectedAppId(e.target.value)}
        >
          <option value="">-- Select Application ID --</option> {/* Default option */}
          {Array.isArray(appIds) && appIds.length > 0 ? (
            appIds.map((appId) => (
              <option key={appId} value={appId}>
                {appId}
