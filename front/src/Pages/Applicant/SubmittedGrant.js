...
export default function SubmittedGrant() {
  ...
  //   // Trigger a GET request to fetch the agreement file from the server
  //   axios
  //     .get(`/downloadAgreement.php?grantId=${grantId}`, {
  //       responseType: 'blob', // Ensure we receive the file as a blob
  //     })
  //     .then((response) => {
  //       // Create a link element to trigger the download
  //       const link = document.createElement('a');
  //       const blob = new Blob([response.data], { type: 'application/pdf' }); // Assuming the file is PDF
  //       link.href = URL.createObjectURL(blob);
  //       link.download = `agreement_${grantId}.pdf`; // Set the file name
  //       link.click();
  //     })
  //     .catch((error) => {
